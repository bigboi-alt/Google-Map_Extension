(function() {
  ...
  if (window._mfLoaded) { const old = document.getElementById('mf-host'); if (old) old.remove(); }
  ...
  const CSS = [
    ...
    '.mfRCMeta{display:flex;gap:12px;font-size:11.5px;color:#4a4a62;margin-bottom:10px;align-items:center;flex-wrap:wrap}',
    '.mfDist{color:#8b5cf6!important;font-weight:600}',
  ...
  function createPanel() {
    ...
    panel.innerHTML = '<div id="mfHeader"><div id="mfDrag"><div id="mfBrand"><div id="mfLogoBox">' + ic.map + '</div><span id="mfTitle">MapFilter</span><span id="mfBadge">PRO</span></div><div id="mfWinBtns"><button class="mfWBtn" id="mfMinBtn">' + ic.min + '</button><button class="mfWBtn" id="mfCloseBtn">' + ic.close + '</button></div></div><div id="mfTabs"><button class="mfTab mfTActive" data-tab="scanner">Scanner</button><button class="mfTab" data-tab="notes">Saved Notes <span id="mfNCount" style="margin-left:4px;color:#a78bfa;font-size:11px">0</span></button></div></div>'
      + '<div id="mfBody"><div id="mfViewScanner" class="mfVActive"><div id="mfSearch"><div class="mfField"><span class="mfFSvg">' + ic.search + '</span><input type="text" id="mfQuery" placeholder="Business type (plumber, dentist, cafe...)"></div><div class="mfField"><span class="mfFSvg">' + ic.pin + '</span><input type="text" id="mfLocation" placeholder="Location (Austin TX, London...)"><button id="mfGps">' + ic.gps + '</button></div><div id="mfRadiusBox"><div id="mfRadiusTop"><span id="mfRadiusLabel">Radius</span><span id="mfRadiusVal">5 km</span></div><input type="range" id="mfRadiusSlider" min="1" max="100" value="5"><div id="mfRadiusMarks"><span>1</span><span>25</span><span>50</span><span>75</span><span>100km</span></div></div><button id="mfScanBtn"><span id="mfScanIcon">' + ic.scan + '</span><span id="mfScanText">Scan Area</span></button><div id="mfProg" style="display:none"><div id="mfProgTrack"><div id="mfProgBar"></div></div><div id="mfProgText">Preparing scan...</div></div></div>'
      ...
      + '<div id="mfHGPrompt"><div id="mfHGPromptTitle">' + ic.bolt + ' Quick Area Scan</div><div class="mfField" style="margin-bottom:12px"><span class="mfFSvg">' + ic.pin + '</span><input type="text" id="mfHGLocation" placeholder="Where? (city, area...)"><button id="mfHGGps">' + ic.gps + '</button></div><div id="mfRadiusBox" style="margin-bottom:12px"><div id="mfRadiusTop"><span id="mfRadiusLabel">Radius</span><span id="mfHGRadiusVal">10 km</span></div><input type="range" id="mfHGRadiusSlider" min="1" max="100" value="10"></div><button id="mfHGScanBtn">Scan for Hot Gigs</button><button id="mfHGCancel">Cancel</button></div>'
      ...
      + '<div id="mfFilters" style="display:none"><details class="mfFB" open><summary><span class="mfFBIcon">' + ic.globe + '</span>Website Filter</summary><div class="mfFBody"><label class="mfChk"><span>No website</span><input type="checkbox" data-f="no-website"></label><label class="mfChk"><span>Has website</span><input type="checkbox" data-f="has-website"></label><label class="mfChk"><span>Social media only</span><input type="checkbox" data-f="social-only"></label></div></details><details class="mfFB"><summary><span class="mfFBIcon">' + ic.chat + '</span>Reviews Filter</summary><div class="mfFBody"><label class="mfChk"><span>No reviews</span><input type="checkbox" data-f="no-reviews"></label><label class="mfChk"><span>Under 10 reviews</span><input type="checkbox" data-f="few-reviews"></label></div></details><details class="mfFB"><summary><span class="mfFBIcon">' + ic.phone + '</span>Contact Filter</summary><div class="mfFBody"><label class="mfChk"><span>Has phone number</span><input type="checkbox" data-f="has-phone"></label><label class="mfChk"><span>No phone number</span><input type="checkbox" data-f="no-phone"></label></div></details><details class="mfFB"><summary><span class="mfFBIcon">' + ic.clip + '</span>Profile Filter</summary><div class="mfFBody"><label class="mfChk"><span>Incomplete profile</span><input type="checkbox" data-f="incomplete"></label><label class="mfChk"><span>No photos</span><input type="checkbox" data-f="no-photos"></label></div></details></div>'
      + '<div id="mfActions" style="display:none"><button id="mfApplyBtn">Apply Filters</button><button id="mfClearBtn">Clear All</button></div>'
      + '<div id="mfResultsWrap" style="display:none"><div class="mfListBar"><span>Showing <b id="mfRCount">0</b> results</span><button class="mfSmlBtn" id="mfExportBtn">' + ic.dl + ' Export CSV</button></div><div id="mfRList" class="mfCList"></div></div>'
      + '</div><div id="mfViewNotes"><div class="mfListBar"><span>You have <b id="mfNCountFull">0</b> saved notes</span><button class="mfSmlBtn" id="mfExportNotesBtn">' + ic.dl + ' Export</button></div><div id="mfNList" class="mfCList"></div></div></div>'
      + '<div id="mfToast"></div>';
  ...
  function bindEvents() {
    var panel = $id('mf-panel-v4');
    var drag = $id('mfDrag');
    var isDrag = false, sX, sY, sL, sT;
    drag.addEventListener('mousedown', function(e) { if (e.target.closest('.mfWBtn')) return; isDrag = true; sX = e.clientX; sY = e.clientY; var r = panel.getBoundingClientRect(); sL = r.left; sT = r.top; panel.style.transition = 'none'; });
    document.addEventListener('mousemove', function(e) { if (!isDrag) return; panel.style.left = (sL + e.clientX - sX) + 'px'; panel.style.top = (sT + e.clientY - sY) + 'px'; panel.style.right = 'auto'; });
    document.addEventListener('mouseup', function() { isDrag = false; panel.style.transition = ''; });
    $id('mfCloseBtn').onclick = function() { panel.classList.add('mfHidden'); };
    $id('mfMinBtn').onclick = function() { panel.classList.toggle('mfMin'); };
    $all('.mfTab').forEach(function(t) { t.onclick = function() { $all('.mfTab').forEach(function(b) { b.classList.remove('mfTActive'); }); t.classList.add('mfTActive'); $id('mfViewScanner').classList.remove('mfVActive'); $id('mfViewNotes').classList.remove('mfVActive'); if (t.dataset.tab === 'scanner') { $id('mfViewScanner').classList.add('mfVActive'); } else { $id('mfViewNotes').classList.add('mfVActive'); renderNotes(); } }; });
    $id('mfRadiusSlider').oninput = function(e) { $id('mfRadiusVal').textContent = e.target.value + ' km'; };
    $id('mfGps').onclick = function() { gpsFill($id('mfLocation')); };
    $id('mfScanBtn').onclick = function() { if (isScanning) { cancelScan = true; } else { startScan(false); } };
    $id('mfQuery').onkeydown = function(e) { if (e.key === 'Enter') startScan(false); };
    $id('mfLocation').onkeydown = function(e) { if (e.key === 'Enter') startScan(false); };
    $id('mfHotGigs').onclick = function() { var q = $id('mfQuery').value.trim(); if (allBiz.length && q) { applyHotGigs(); toast('Hot Gigs applied.', 'ok'); return; } var prompt = $id('mfHGPrompt'); var loc = $id('mfLocation').value.trim(); if (loc) $id('mfHGLocation').value = loc; prompt.style.display = (prompt.style.display === 'none' ? 'block' : 'none'); };
    $id('mfHGRadiusSlider').oninput = function(e) { $id('mfHGRadiusVal').textContent = e.target.value + ' km'; };
    $id('mfHGGps').onclick = function() { gpsFill($id('mfHGLocation')); };
    $id('mfHGCancel').onclick = function() { $id('mfHGPrompt').style.display = 'none'; };
    $id('mfHGScanBtn').onclick = function() { var loc = $id('mfHGLocation').value.trim(); if (!loc) { toast('Please enter a location first.', 'err'); return; } $id('mfHGPrompt').style.display = 'none'; $id('mfLocation').value = loc; var r = $id('mfHGRadiusSlider').value; $id('mfRadiusSlider').value = r; $id('mfRadiusVal').textContent = r + ' km'; $id('mfQuery').value = 'businesses'; startScan(true); };
    $all('.mfChip').forEach(function(chip) { chip.onclick = function() { $all('.mfChip').forEach(function(c) { c.classList.remove('mfChipOn'); }); chip.classList.add('mfChipOn'); $all('[data-f]').forEach(function(cb) { cb.checked = false; }); var p = chip.dataset.p; if (p === 'no-website') fc('no-website'); else if (p === 'no-reviews') fc('no-reviews'); else if (p === 'weak') { fc('no-website'); fc('few-reviews'); } else if (p === 'leads') { fc('no-website'); fc('has-phone'); } applyFilters(); }; });
    $all('[data-f]').forEach(function(cb) { cb.onchange = applyFilters; });
    $id('mfApplyBtn').onclick = function() { applyFilters(); highlightResults(); toast(filtered.length + ' businesses highlighted', 'ok'); };
    $id('mfClearBtn').onclick = function() { $all('[data-f]').forEach(function(cb) { cb.checked = false; }); $all('.mfChip').forEach(function(c) { c.classList.remove('mfChipOn'); }); $q('[data-p="all"]').classList.add('mfChipOn'); applyFilters(); toast('Filters cleared', 'info'); };
    $id('mfExportBtn').onclick = function() { exportCSV(filtered, 'mapfilter-scan'); };
    $id('mfExportNotesBtn').onclick = function() { exportCSV(savedNotes, 'mapfilter-notes'); };
    panel.addEventListener('click', function(e) { var a = e.target.closest('[data-action]'); if (!a) return; var act = a.dataset.action; if (act === 'openBiz') openBiz(a.dataset.name); else if (act === 'toggleNote') toggleNote(a.dataset.idx); else if (act === 'openMapUrl') openMapUrl(a.dataset.url); else if (act === 'delNote') delNote(a.dataset.id); });
    panel.addEventListener('input', function(e) { var a = e.target.closest('[data-action="saveNote"]'); if (!a) return; saveNoteText(a.dataset.id, a); });
  }

  function gpsFill(inp) { inp.value = 'Locating...'; navigator.geolocation.getCurrentPosition(function(p) { inp.value = p.coords.latitude.toFixed(4) + ', ' + p.coords.longitude.toFixed(4); toast('Location acquired', 'ok'); }, function() { inp.value = ''; toast('Type location manually', 'info'); }, { enableHighAccuracy: true, timeout: 10000 }); }
  function fc(n) { var el = $q('[data-f="' + n + '"]'); if (el) el.checked = true; }
  function getRadius() { return parseInt($id('mfRadiusSlider').value) || 5; }

  function getSearchAreas(q, loc, km) {
    var a = [q + ' in ' + loc]; if (km <= 5) return a;
    var dirs = ['north','south','east','west'], subDirs = ['northeast','northwest','southeast','southwest'];
    if (km > 5) dirs.forEach(function(d) { a.push(q + ' in ' + d + ' ' + loc); });
    if (km > 15) subDirs.forEach(function(d) { a.push(q + ' in ' + d + ' ' + loc); });
    if (km >= 30) { for (var dist = 20; dist <= km; dist += 20) { dirs.concat(subDirs).forEach(function(d) { a.push(q + ' ' + dist + 'km ' + d + ' of ' + loc); }); } }
    return a.filter(function(v, i, s) { return s.indexOf(v) === i; });
  }

  async function startScan(hotGigs) {
    var query = $id('mfQuery').value.trim(), location = $id('mfLocation').value.trim();
    ...
    if (!userLat) {
      await new Promise(function(res) {
        navigator.geolocation.getCurrentPosition(
          function(p) { userLat = p.coords.latitude; userLng = p.coords.longitude; res(); },
          function() { res(); },
          { timeout: 5000 }
        );
      });
    ...
    var radius = getRadius(), searches = getSearchAreas(query, location, radius);
    allBiz = []; var seen = new Set();
    ...
    try {
      for (var i = 0; i < searches.length; i++) {
        ...
        var ok = await doSearch(searches[i]); if (!ok) continue;
        await wait(2000); await scrollFeed();
        var cards = getCards();
        for (var j = 0; j < cards.length; j++) {
          if (cancelScan) break;
          var basic = readCard(cards[j]);
          if (!basic.name || seen.has(basic.name)) continue;
          seen.add(basic.name);
          try { Object.assign(basic, await clickAndRead(cards[j])); } catch(e) {}
          if (basic.website) { var soc = ['facebook.com','instagram.com','twitter.com','x.com','linkedin.com','tiktok.com','youtube.com']; basic.isSocialOnly = soc.some(function(d) { return basic.website.toLowerCase().indexOf(d) !== -1; }); } else { basic.isSocialOnly = false; }
          var clean = Object.assign({}, basic); delete clean.el;
          clean.id = btoa(unescape(encodeURIComponent(clean.name + (clean.phone||'') + (clean.rating||'')))).substring(0, 20);
          allBiz.push(clean);
        }
      }
      safeSave({ lastScan: allBiz, lastScanTime: Date.now(), lastQuery: query, lastLocation: location });
      showResults();
      if (hotGigs) { applyHotGigs(); } else { applyFilters(); }
      if (!cancelScan) toast('Done. ' + allBiz.length + ' indexed.', 'ok');
    } catch(e) {
      if (allBiz.length) {
        safeSave({ lastScan: allBiz, lastScanTime: Date.now() });
        showResults(); applyFilters(); toast('Partial scan saved.', 'ok');
      } else {
        toast('Scan failed: ' + e.message, 'err');
      }
    }
    isScanning = false; cancelScan = false;
    btn.classList.remove('scanning'); $id('mfScanText').textContent = 'Scan Area'; $id('mfScanIcon').innerHTML = ic.scan; $id('mfProg').style.display = 'none';
  }

  function updateProgress(c, t, txt) { $id('mfProgBar').style.width = (Math.round((c/t)*90)+5)+'%'; $id('mfProgText').textContent = 'Zone '+c+'/'+t+' - '+(txt||''); }

  async function doSearch(query) {
    var sb = null; var selectors = ['#searchboxinput','input[aria-label="Search Google Maps"]','input[name="q"]']; for (var s = 0; s < selectors.length; s++) { sb = document.querySelector(selectors[s]); if (sb) break; } if (!sb) return false;
    sb.focus(); await wait(200); sb.select(); await wait(100); sb.value = ''; sb.dispatchEvent(new Event('input',{bubbles:true})); await wait(100);
    var set = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; set.call(sb,query);
    ...
    if (sb.value !== query) { sb.value=''; sb.focus(); for (var ch = 0; ch < query.length; ch++) { sb.value+=query[ch]; sb.dispatchEvent(new InputEvent('input',{bubbles:true,data:query[ch],inputType:'insertText'})); await wait(10); } await wait(200); }
    sb.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true})); await wait(200);
    var form=sb.closest('form'); if(form) try{form.dispatchEvent(new Event('submit',{bubbles:true}))}catch(e){} await wait(200);
    var btns = ['#searchbox-searchbutton','button[aria-label="Search"]']; for (var b = 0; b < btns.length; b++) { var btn = document.querySelector(btns[b]); if(btn){btn.click();break;} }
    var att=0; while(att<20){await wait(300);if(getCards().length>0){await wait(600);return true;}att++;} return false;
  }

  async function scrollFeed() {
    var feed=null; var fsel=['div[role="feed"]','.m6QErb.DxyBCb','.m6QErb.WNBkOb','.m6QErb']; for(var s=0;s<fsel.length;s++){feed=document.querySelector(fsel[s]);if(feed)break;} if(!feed)return;
    var prev=0,stale=0; while(stale<3){feed.scrollTop=feed.scrollHeight;await wait(600);if(feed.scrollHeight===prev)stale++;else stale=0;prev=feed.scrollHeight;if(getCards().length>=120)break;} feed.scrollTop=0;await wait(150);
  }

  function getCards() { var csel=['div.Nv2PK','a.hfpxzc','div[jsaction*="mouseover:pane"]']; for(var s=0;s<csel.length;s++){var f=document.querySelectorAll(csel[s]);if(f.length)return Array.from(f);}return[]; }

  function readCard(card) {
    var info={name:'',category:'',rating:null,reviewCount:0,address:'',phone:'',website:'',isOpen:null,photoCount:0,hasHours:false,mapsUrl:'',lat:null,lng:null,el:card};
    var nsel=['.qBF1Pd','.fontHeadlineSmall','.NrDZNb']; for(var s=0;s<nsel.length;s++){var el=card.querySelector(nsel[s]);if(el&&el.textContent.trim()){info.name=el.textContent.trim();break;}}
    if(!info.name)info.name=(card.getAttribute('aria-label')||'').trim();
    var rat=card.querySelector('.MW4etd');if(rat)info.rating=parseFloat(rat.textContent)||null;
    var rev=card.querySelector('.UY7F9');if(rev){var rm=rev.textContent.match(/([\d,]+)/);if(rm)info.reviewCount=parseInt(rm[1].replace(/,/g,''));}
    card.querySelectorAll('.W4Efsd').forEach(function(s){var t=s.textContent.trim();if(t.indexOf('·')!==-1)t.split('·').map(function(p){return p.trim();}).forEach(function(p){if(p&&!p.match(/^[\d.]+$/)&&!p.match(/\(\d/)&&p.length<40&&!info.category)info.category=p;});if(t.toLowerCase().indexOf('open')!==-1)info.isOpen=true;if(t.toLowerCase().indexOf('closed')!==-1)info.isOpen=false;});
    var link=card.querySelector('a[href*="maps/place"]')||card.querySelector('a[href*="@"]');
    if(link){var c=extractCoords(link.href);if(c){info.lat=c.lat;info.lng=c.lng;}}
    return info;
  }
  ...
  async function clickAndRead(card) {
    ...
    var att=0; while(att<20){await wait(200);if(window.location.href!==urlBefore)break;att++;}
    await wait(400);
    att=0; while(att<30){if(document.querySelector('button[data-item-id="address"]')||document.querySelector('a[data-item-id="authority"]')||document.querySelector('button[data-item-id*="phone"]')||document.querySelector('.DkEaL')||document.querySelector('h1.DUwDvf'))break;await wait(200);att++;}
    var wsel=['a[data-item-id="authority"]','a[aria-label*="ebsite"]','a[data-tooltip="Open website"]']; for(var s=0;s<wsel.length;s++){var el=document.querySelector(wsel[s]);if(el&&el.href){d.website=el.href;break;}}
    var psel=['button[data-item-id*="phone"]','[data-tooltip="Copy phone number"]','button[aria-label*="Phone"]']; for(var s=0;s<psel.length;s++){var el=document.querySelector(psel[s]);if(el){var m=(el.getAttribute('aria-label')||el.textContent||'').match(/[\d\s\-().+]{7,}/);if(m){d.phone=m[0].trim();break;}}}
    var asel=['button[data-item-id="address"]','[data-tooltip="Copy address"]']; for(var s=0;s<asel.length;s++){var el=document.querySelector(asel[s]);if(el){d.address=(el.getAttribute('aria-label')||el.textContent||'').replace(/^Address:\s*/i,'').trim();break;}}
    d.hasHours=!!(document.querySelector('[data-item-id="oh"]')||document.querySelector('.t39EBf'));
    var ph=document.querySelector('button[aria-label*="photo"]');if(ph){var pm=(ph.getAttribute('aria-label')||'').match(/([\d,]+)/);if(pm)d.photoCount=parseInt(pm[1].replace(/,/g,''));}
    var cat=document.querySelector('.DkEaL');if(cat)d.category=cat.textContent.trim()||'';
    d.mapsUrl=window.location.href;
    var coords=extractCoords(window.location.href);if(coords){d.lat=coords.lat;d.lng=coords.lng;}
    var bsel=['button[aria-label="Back"]','button[jsaction*="back"]']; for(var s=0;s<bsel.length;s++){var btn=document.querySelector(bsel[s]);if(btn){btn.click();await wait(600);var w=0;while(w<12&&getCards().length===0){await wait(200);w++;}if(getCards().length===0){document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',code:'Escape',keyCode:27,bubbles:true}));await wait(500);}break;}}
    return d;
  }

  function showResults() {
    ['mfStats','mfChips','mfFilters','mfActions','mfResultsWrap'].forEach(function(id){$id(id).style.display='';});
    animN('mfSTot',allBiz.length); animN('mfSNoWeb',allBiz.filter(function(b){return !b.website;}).length); animN('mfSNoPh',allBiz.filter(function(b){return !b.phone;}).length);
  }
  function animN(id,target){var el=$id(id);var c=0;var s=Math.max(1,Math.ceil(target/20));var t=setInterval(function(){c+=s;if(c>=target){c=target;clearInterval(t);}el.textContent=c;},30);}

  function applyFilters() {
    var f={}; $all('[data-f]:checked').forEach(function(cb){f[cb.dataset.f]=true;});
    filtered=allBiz.filter(function(b){if(f['no-website']&&b.website)return false;if(f['has-website']&&!b.website)return false;if(f['social-only']&&!b.isSocialOnly)return false;if(f['no-reviews']&&(b.reviewCount||0)>0)return false;if(f['few-reviews']&&(b.reviewCount||0)>=10)return false;if(f['has-phone']&&!b.phone)return false;if(f['no-phone']&&b.phone)return false;if(f['incomplete']&&b.website&&b.phone)return false;if(f['no-photos']&&b.photoCount>0)return false;return true;});
    $id('mfSMatch').textContent=filtered.length; renderResults();
  }

  function applyHotGigs(){$all('[data-f]').forEach(function(cb){cb.checked=false;});fc('no-website');fc('has-phone');fc('few-reviews');$all('.mfChip').forEach(function(c){c.classList.remove('mfChipOn');});showResults();applyFilters();highlightResults();}
  function isSaved(id){return savedNotes.some(function(n){return n.id===id;});}

  function toggleNote(idx) {
    var b=filtered[parseInt(idx)]; if(!b) return;
    var exists=savedNotes.findIndex(function(n){return n.id===b.id;});
    if(exists>-1){savedNotes.splice(exists,1);toast('Removed from Notes','info');}
    else{savedNotes.push(Object.assign({},b,{noteText:'',dateSaved:Date.now()}));toast('Lead Saved','ok');}
    safeSave({mfNotes:savedNotes}); refreshNoteCounts(); renderResults();
    if($id('mfViewNotes').classList.contains('mfVActive')) renderNotes();
  }

  function saveNoteText(id,textArea){var idx=savedNotes.findIndex(function(n){return n.id===id;});if(idx>-1){savedNotes[idx].noteText=textArea.value;safeSave({mfNotes:savedNotes});}}

  function delNote(id) {
    if(!id) return; var prev=savedNotes.length;
    savedNotes=savedNotes.filter(function(n){return String(n.id)!==String(id);});
    if(savedNotes.length<prev){safeSave({mfNotes:savedNotes});toast('Lead Trashed','ok');refreshNoteCounts();renderNotes();if($id('mfViewScanner')&&$id('mfViewScanner').classList.contains('mfVActive'))renderResults();}
  }
  ...
  function renderResults() {
    var list=$id('mfRList'); $id('mfRCount').textContent=filtered.length;
    ...
    list.innerHTML=filtered.slice(0,50).map(function(b,i){
      var saved=isSaved(b.id);
      var distHTML=(userLat&&b.lat&&b.lng)?'<span class="mfDist">&#x1F4CD; '+getDistance(userLat,userLng,b.lat,b.lng)+' away</span>':'';
      return '<div class="mfRC">'
        +'<div class="mfRCTop" data-action="openBiz" data-name="'+esc(b.name)+'"><span class="mfRCName">'+esc(b.name)+'</span><span class="mfRCRat">'+(b.rating?'<span class="mfRCStar">&#9733;</span> '+b.rating:'&mdash;')+'</span></div>'
        +'<div class="mfRCCat" data-action="openBiz" data-name="'+esc(b.name)+'">'+esc(b.category||'Uncategorized')+'</div>'
        +'<div class="mfRCMeta">'+(b.phone?'<span>'+esc(b.phone)+'</span>':'')+'<span>'+(b.reviewCount||0)+' reviews</span>'+distHTML+'</div>'
        +'<div class="mfRCBadges">'+(!b.website?'<span class="mfTag mfTR">No Website</span>':'<span class="mfTag mfTG">Has Website</span>')+(!b.phone?'<span class="mfTag mfTR">No Phone</span>':'')+((b.reviewCount||0)===0?'<span class="mfTag mfTY">Zero Reviews</span>':'')+'</div>'
        +'<div class="mfRCActs"><div class="mfRCAct" data-action="openBiz" data-name="'+esc(b.name)+'">'+ic.link+' Profile</div><div class="mfRCAct mfRCNoteAct" data-action="toggleNote" data-idx="'+i+'">'+(saved?ic.star:ic.bolt)+' '+(saved?'Saved':'Save Lead')+'</div></div>'
        +'</div>';
    }).join('');
  }
  ...
  function renderNotes() {
    ...
    list.innerHTML=savedNotes.map(function(b){
      var distHTML=(userLat&&b.lat&&b.lng)?'<span class="mfDist">&#x1F4CD; '+getDistance(userLat,userLng,b.lat,b.lng)+' away</span>':'';
      return '<div class="mfRC" style="border-color:rgba(139,92,246,0.2)">'
        +'<div class="mfRCTop"><span class="mfRCName">'+esc(b.name)+'</span><span class="mfRCRat">'+(b.rating?'<span class="mfRCStar">&#9733;</span> '+b.rating:'&mdash;')+'</span></div>'
        +'<div class="mfRCCat">'+esc(b.category||'Uncategorized')+'</div>'
        +'<div class="mfRCMeta">'+(b.phone?'<span>'+esc(b.phone)+'</span>':'')+distHTML+'</div>'
        ...
        +'<div class="mfRCActs"><div class="mfRCAct" data-action="openMapUrl" data-url="'+b.mapsUrl+'">'+ic.link+' Link</div><div class="mfRCAct mfRCNoteAct" data-action="delNote" data-id="'+b.id+'" style="color:#f87171">'+ic.trash+' Trash</div></div>'
        +'</div>';
    }).join('');
  }

  function openMapUrl(url){if(url)window.open(url,'_blank');}
  function openBiz(name){var cards=getCards();for(var i=0;i<cards.length;i++){var card=cards[i];var el=card.querySelector('.qBF1Pd')||card.querySelector('.fontHeadlineSmall');var n=(el&&el.textContent.trim())||(card.getAttribute('aria-label')||'').trim();if(n===name){(card.querySelector('a.hfpxzc')||card).click();card.scrollIntoView({behavior:'smooth',block:'center'});return;}}}
  function highlightResults(){var names=new Set(filtered.map(function(b){return b.name;}));document.querySelectorAll('.mf-match,.mf-dim').forEach(function(el){el.classList.remove('mf-match','mf-dim');});getCards().forEach(function(card){var el=card.querySelector('.qBF1Pd')||card.querySelector('.fontHeadlineSmall');var n=(el&&el.textContent.trim())||(card.getAttribute('aria-label')||'').trim();if(names.has(n))card.classList.add('mf-match');else if(names.size>0)card.classList.add('mf-dim');});}
  function exportCSV(dataArray,filename){if(!dataArray||!dataArray.length){toast('No records to export','err');return;}var fields=['name','category','address','phone','website','rating','reviewCount','noteText'];var csv=[fields.join(',')].concat(dataArray.map(function(r){return fields.map(function(f){return '"'+String(r[f]||'').replace(/"/g,'""')+'"';}).join(',');})).join('\n');var blob=new Blob([csv],{type:'text/csv'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=filename+'.csv';a.click();toast('CSV generated','ok');}
  async function loadSaved(){try{var data=await chrome.storage.local.get(['lastScan','lastQuery','lastLocation']);if(data.lastScan&&data.lastScan.length){allBiz=data.lastScan;if(data.lastQuery)$id('mfQuery').value=data.lastQuery;if(data.lastLocation)$id('mfLocation').value=data.lastLocation;showResults();applyFilters();}}catch(e){}}
  async function loadNotes(){try{var data=await chrome.storage.local.get(['mfNotes']);if(data.mfNotes){savedNotes=data.mfNotes;refreshNoteCounts();}}catch(e){}}
  function toggleMapFilter(){var host=document.getElementById('mf-host');if(!host){createPanel();return;}var panel=$id('mf-panel-v4');if(panel)panel.classList.toggle('mfHidden');}
  function toast(msg,type){var t=$id('mfToast');if(!t)return;t.className=(type==='ok'?'mfTOk':(type==='err'?'mfTErr':'mfTInfo'));t.textContent=msg;requestAnimationFrame(function(){t.classList.add('mfTShow');});setTimeout(function(){t.classList.remove('mfTShow');},2500);}
